...
class JobUpdateEventSubscriber extends AbstractIdleService implements PubsubEvent.EventSubscriber {
  ...
  public void taskChangedState(TaskStateChange change) {
    // Restart mechanism will create a new task with a different ID while
    // killing the existing one. This will cause the updating mechanism to fail since it will get
    // an update for this task for both the kill and the create. The kill will make it seem like
    // the update failed but this is the expected behavior. Therefore we trap the killed status
    // if it is coming right after a restart status and make it a NOOP.
    ...
    try {
      // TODO(rdelvalle): Delete before final cut
      LOG.info("{} id: {} transitioned from {} to {}",
          change.getTask().getAssignedTask().getTask().getJob(),
          change.getTask().getAssignedTask().getInstanceId(),
          change.getOldState(),
          change.getNewState());
